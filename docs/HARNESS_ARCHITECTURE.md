...
For failed and blocked outcomes, `$work` records feedback before the next attempt. The feedback file carries the repair hypothesis and repo-relative evidence refs. `--retry-of <run-id>` starts a sibling run that links to the old evidence instead of overwriting it.
