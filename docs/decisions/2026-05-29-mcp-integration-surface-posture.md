...
5. Preserve the zero-runtime-dependency posture for now; do not adopt `@modelcontextprotocol/sdk` until a future ADR proves the compatibility benefit is worth making Open Scaffold's root package depend on a protocol SDK.

Short version: yes to MCP as an integration facet; no to MCP as the execution controller; no new write/spawn authority now.

## Decision drivers

- `060-mcp-server` already shipped a zero-dependency stdio server, so the question is not whether MCP exists; it is whether Open Scaffold should make it stable and strategic.
...
- Slice `130-section-parser-canonical-contract` intentionally protected the zero-dependency parser/core posture. Pulling in `@modelcontextprotocol/sdk` would be the first hard runtime dependency and needs explicit justification.
...
| Protocol loop | Hand-rolled zero-dependency JSON-RPC stdio loop. | Need protocol conformance fixtures; SDK adoption remains an ADR question, not a default. |
...
| Adopt `@modelcontextprotocol/sdk` immediately | Better protocol alignment; future updates easier | First hard runtime dependency; package footprint/security review; migration churn without proven need | Defer to future ADR |
...
- The zero-dependency root-package posture remains intact until there is source-grounded evidence that the official SDK is worth adopting.
