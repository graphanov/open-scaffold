...
function downstreamReadmeTemplate(): string {
  return [
    ...
    '2. Create the current slice with `osc plan new <slug> --stage active`, then fill every TODO prompt.',
    '3. Do the work and run the project checks.',
    '4. Record evidence with `osc evidence new <slug>`, then replace every TODO with real commands and results.',
...
function downstreamAgentInstructionsTemplate(file: 'AGENTS.md' | 'CLAUDE.md'): string {
  return [
    ...
    '',
    'Then update `MISSION.md` and create the first active plan with `osc plan new <slug> --stage active` or by copying `.osc/plans/handoff-template.md`.',
  ].join('\n') + '\n';
}
...
function downstreamMinimumScaffoldDocTemplate(): string {
  return [
    ...
    '2. Create one active plan with `osc plan new <slug> --stage active`, then fill every TODO prompt with real acceptance criteria and verification.',
    '3. Execute the work and run the project checks.',
    '4. Run `./verify.sh --standard` before calling the slice done.',
    '5. Record evidence with `osc evidence new <slug>`, replace every TODO with real results, then close the plan with `./close.sh <plan-slug>`.',
    '',
