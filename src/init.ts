...
function downstreamReadmeTemplate(): string {
  return [
    ...
    '2. Create the current slice with `npx open-scaffold plan new <slug> --stage active` (or `osc plan new <slug> --stage active` if installed locally), then fill every TODO prompt. Shell fallback: `cp .osc/plans/handoff-template.md .osc/plans/active/<slug>.md`.',
...
function downstreamMinimumScaffoldDocTemplate(): string {
  return [
    ...
    '5. Record evidence with `npx open-scaffold evidence new <slug>` (or `osc evidence new <slug>` if installed locally), replace every TODO with real results, then close the plan with `./close.sh <plan-slug>`. Shell fallback: create a note manually under `.osc/releases/`.',
    '',
