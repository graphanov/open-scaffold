...
function evidenceMarkdown(slug: string, planPath: string): string {
  return [
    ...
    '## Summary',
    '',
    'First-run evidence skeleton created by `osc first-run`.',
    '',
    '## Traceability',
    '',
    `- Plan: \`${planPath}\``,
    ...
    '## Boundary statement',
    '',
    'Structural-only, local-only skeleton. This does not prove semantic correctness, compliance, deployment readiness, or runtime execution.',
    '',
  ].join('\n');
}
