...
export function compareProofManifest(pathArg: string) {
  ...
  const metrics = ((raw.metrics as unknown[]) ?? []).map((metric, index) => proofMetric(metric, index, manifestDir, [], [])).filter((metric): metric is ProofMetric => Boolean(metric)).map((metric) => { const winner = proofWinner(metric), improvementRatio = proofRatio(metric, winner), minimumRatio = metric.minimum_ratio ?? null, minimumRatioPassed = minimumRatio === null ? null : winner === 'scaffolded' && improvementRatio !== null && improvementRatio >= minimumRatio; return { ...metric, winner, delta: Number((metric.scaffolded - metric.control).toFixed(6)), improvementRatio, minimumRatio, minimumRatioPassed, sourceRefs: metric.source_refs }; });
