...
export function compareProofManifest(pathArg: string) {
  ...
  const boundedProof = categories.quality !== 'missing' && categories.quality !== 'regressed' && categories.evolution === 'improved' && categories.tokens !== 'regressed' && categories.speed !== 'regressed' && controlWins === 0;
  return { schema: PROOF_COMPARISON_RESULT_SCHEMA, comparisonId: asString(raw.comparison_id) ?? 'proof-comparison', title: asString(raw.title) ?? 'Proof comparison', question: asString(raw.question) ?? 'Does the scaffolded arm improve the bounded comparison?', arms: { control: proofArm(isRecord(raw.arms) ? raw.arms.control : null, 'control'), scaffolded: proofArm(isRecord(raw.arms) ? raw.arms.scaffolded : null, 'scaffolded') }, metrics, summary: { scaffoldedWins, controlWins, ties, categories, boundedProof, verdict: boundedProof ? 'PASS: bounded source-labeled comparison preserves decision quality and improves evolution without token/speed regression.' : 'FAIL/INCONCLUSIVE: this bounded comparison does not show the scaffolded arm is better; inspect metric regressions or missing categories.' }, caveats: proofStrings(raw.caveats).length > 0 ? proofStrings(raw.caveats) : ['Bounded comparison only; not a universal benchmark or model-ranking claim.'], validation };
}
