...
const proofRatio = (metric: ProofMetric, winner: ProofWinner): ProofRatio => { if (winner === 'tie') return 1; const baseline = winner === 'scaffolded' ? metric.control : metric.scaffolded, improved = winner === 'scaffolded' ? metric.scaffolded : metric.control; if (metric.direction === 'higher') { if (baseline === 0) return improved > 0 ? 'unbounded' : null; return Number((improved / baseline).toFixed(6)); } if (improved === 0) return baseline > 0 ? 'unbounded' : null; return Number((baseline / improved).toFixed(6)); };
...
export function compareProofManifest(pathArg: string) {
  ...
  const metrics = ((raw.metrics as unknown[]) ?? []).map((metric, index) => proofMetric(metric, index, manifestDir, [], [])).filter((metric): metric is ProofMetric => Boolean(metric)).map((metric) => { const winner = proofWinner(metric), improvementRatio = proofRatio(metric, winner), minimumRatio = metric.minimum_ratio ?? null, minimumRatioPassed = proofRatioPasses(improvementRatio, minimumRatio, winner); return { ...metric, winner, delta: Number((metric.scaffolded - metric.control).toFixed(6)), improvementRatio, minimumRatio, minimumRatioPassed, sourceRefs: metric.source_refs }; });
