...
const proofRawRatio = (metric: ProofMetric, winner: ProofWinner): ProofRatio => { if (winner === 'tie') return 1; const baseline = winner === 'scaffolded' ? metric.control : metric.scaffolded, improved = winner === 'scaffolded' ? metric.scaffolded : metric.control; if (metric.direction === 'higher') { if (baseline === 0) return improved > 0 ? 'unbounded' : null; return improved / baseline; } if (improved === 0) return baseline > 0 ? 'unbounded' : null; return baseline / improved; };
const proofRatio = (metric: ProofMetric, winner: ProofWinner): ProofRatio => { const ratio = proofRawRatio(metric, winner); return typeof ratio === 'number' ? Number(ratio.toFixed(6)) : ratio; };
