...
function proofRequiredEvidence(raw: Record<string, unknown>, failures: ProofIssue[]): string[] {
  if (!Object.hasOwn(raw, 'required_evidence')) return [];
  if (!Array.isArray(raw.required_evidence)) { failures.push(proofIssue('fail', 'invalid-required-evidence', 'required_evidence must be an array of evidence IDs when present.', 'required_evidence')); return []; }
  const ids = raw.required_evidence.filter((id): id is string => typeof id === 'string' && id.trim().length > 0).map((id) => id.trim());
  if (ids.length !== raw.required_evidence.length) failures.push(proofIssue('fail', 'invalid-required-evidence-id', 'Each required_evidence entry must be a non-empty string.', 'required_evidence'));
  const dupes = ids.filter((id, index) => ids.indexOf(id) !== index);
  if (dupes.length > 0) failures.push(proofIssue('fail', 'duplicate-required-evidence-id', `required_evidence must not contain duplicate IDs: ${[...new Set(dupes)].join(', ')}`, 'required_evidence'));
  return ids;
}
function validateProofRaw(raw: unknown, manifestDir: string): ProofValidationResult {
  ...
  const evidenceItems = proofEvidenceBattery(raw, manifestDir, failures), evidenceIds = new Set(evidenceItems.map((item) => item.id));
  for (const id of proofRequiredEvidence(raw, failures)) if (!evidenceIds.has(id)) failures.push(proofIssue('fail', 'missing-required-evidence-row', `required_evidence ID is not present in evidence_battery: ${id}`, 'required_evidence'));
...
export function compareProofManifest(pathArg: string) {
  ...
  const evidenceBattery = proofEvidenceBattery(raw, manifestDir, []), requiredEvidence = proofStrings(raw.required_evidence), evidenceBatteryBlocking = evidenceBattery.filter((item) => item.requiredForPass && !PROOF_EVIDENCE_PASS_STATUSES.has(item.status)).map((item) => item.id), evidenceBatteryApplicable = evidenceBattery.length > 0 || requiredEvidence.length > 0, evidenceBatteryPresent = evidenceBattery.length > 0, evidenceBatteryPass: boolean | null = !evidenceBatteryApplicable ? null : evidenceBatteryBlocking.length === 0, evidenceBatteryStatus: 'pass' | 'fail' | 'not_evaluated' = !evidenceBatteryApplicable ? 'not_evaluated' : evidenceBatteryBlocking.length === 0 ? 'pass' : 'fail';
...
export function renderProofComparison(result: ReturnType<typeof compareProofManifest>, format: ProofRenderFormat = 'terminal'): string {
  ...
  const batteryStatusLabel = result.summary.evidenceBatteryStatus === 'not_evaluated' ? 'not evaluated' : result.summary.evidenceBatteryStatus;
  const batteryLines = result.evidenceBattery.length > 0 ? [
    '',
    format === 'markdown' ? '## Proof battery' : 'Proof battery',
    `Evidence-battery gate: ${batteryStatusLabel}${result.summary.evidenceBatteryBlocking.length > 0 ? ` (blocking: ${result.summary.evidenceBatteryBlocking.join(', ')})` : ''}`,
  ...
  ] : ['', format === 'markdown' ? '## Proof battery' : 'Proof battery', `Evidence-battery gate: ${batteryStatusLabel} (no evidence_battery rows supplied; the gate was not evaluated for this manifest)`];
