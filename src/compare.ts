...
export function compareProofManifest(pathArg: string) {
  ...
  const evidenceBattery = proofEvidenceBattery(raw, manifestDir, []), evidenceBatteryPresent = evidenceBattery.length > 0, evidenceBatteryBlocking = evidenceBattery.filter((item) => item.requiredForPass && !PROOF_EVIDENCE_PASS_STATUSES.has(item.status)).map((item) => item.id), evidenceBatteryPass: boolean | null = !evidenceBatteryPresent ? null : evidenceBatteryBlocking.length === 0, evidenceBatteryStatus: 'pass' | 'fail' | 'not_evaluated' = !evidenceBatteryPresent ? 'not_evaluated' : evidenceBatteryBlocking.length === 0 ? 'pass' : 'fail';
