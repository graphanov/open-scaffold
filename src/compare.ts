...
export function renderProofComparison(result: ReturnType<typeof compareProofManifest>, format: ProofRenderFormat = 'terminal'): string {
  ...
  const requiredSet = new Set(result.summary.evidenceBatteryRequired ?? []);
  const batteryLines = result.evidenceBattery.length > 0 ? [
    ...
    '|---|---|---|---|---|---|---|',
    ...result.evidenceBattery.map((item) => `| ${item.id} | ${item.kind} | ${item.status} | ${item.requiredForPass || requiredSet.has(item.id) ? 'required' : 'disclosure-only'} | ${escapeCell(item.claim)} | ${escapeCell(item.boundary)} | ${item.sourceRefs.join('<br>')} |`),
