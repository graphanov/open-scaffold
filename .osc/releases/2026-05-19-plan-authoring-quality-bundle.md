...
- Pinpoint dogfood fix: `node dist/cli.js plan validate .osc/plans/done/055-plan-linter.md --strict` initially failed because literal `` `TODO:` `` examples and non-blocking open questions were treated as unresolved placeholders/blockers; after the fix, text output returns `0 issues found` and `--json --strict` returns `[]`. The same fix set also accepts status text that starts with the correct stage token, e.g. `backlog — planned after dependency lands`, while still rejecting substring false positives such as `inactive` in `active/`.
- Package dry-run: `npm pack --dry-run --json` — pass; candidate `open-scaffold-0.4.5.tgz`, 95 files, unpacked size 594,718 bytes, includes `.osc/plans/templates/{README.md,arch-decision.md,bug-fix.md,dependency-upgrade.md,docs-update.md,new-feature.md,refactor.md}`.
